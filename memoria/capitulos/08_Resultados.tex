...
Conviene aclarar dos de estos indicadores. El \textit{TTFT} (\textit{Time To First Token}) es el tiempo que transcurre hasta que el usuario ve la primera palabra de la respuesta. La latencia \textit{p95} es el percentil 95: el valor por debajo del cual se resuelve el 95\% de las consultas; se usa en lugar de la media porque refleja mejor la experiencia en los peores casos habituales, no solo el caso promedio.
...
\end{axis}
\end{tikzpicture}
\caption[Descomposición de la latencia por componente (consulta RAG).]{Descomposición de la latencia por componente para una consulta RAG típica. La inferencia del LLM domina el tiempo total del flujo.}
...
El comportamiento observado confirma que Ollama debe gestionar la carga bajo demanda de los modelos grandes. En el despliegue actual, el alias JARVIS (\texttt{rag-qwen-ft:latest}) atiende las consultas factuales y procedurales ($<3$\,s de latencia), mientras que \texttt{qwen2.5:32b-instruct-q4\_K\_M} se activa autom\'aticamente para consultas anal\'iticas gracias al \texttt{QueryProcessor} (v\'ease Secci\'on~\ref{subsec:query-processor}). Adicionalmente, \texttt{qwen2.5-32b} es el modelo utilizado para la summarizaci\'on autom\'atica de conversaciones largas en el \texttt{MemoryManager}, aprovechando su ventana de 32\,K tokens para sintetizar historiales extensos sin perder informaci\'on.
...
\subsection{Query Processor: intent detection, smart routing y query expansion}

\begin{itemize}
    \item \textbf{Prueba A --- Intent detection y routing:} Consulta analítica ``Explica las diferencias entre el procedimiento de auditoría interna y la auditoría externa según los documentos del sistema de calidad.''
    \item \textbf{Resultado:} El \texttt{QueryProcessor} clasificó correctamente la consulta como \texttt{ANALYTICAL}, ajustó el \texttt{top\_k} a 12 y enrutó la generación de respuesta a \texttt{qwen2.5-32b}. La respuesta incorporó fragmentos de múltiples documentos con mayor coherencia argumentativa que la producida por JARVIS en las mismas condiciones.
    \item \textbf{Prueba B --- Query expansion:} Consulta corta ``normas escaleras''.
    \item \textbf{Resultado:} El procesador generó dos variantes adicionales: \textit{``requisitos técnicos escaleras portátiles normativa''} y \textit{``normativa PRL trabajo en altura escaleras industriales''}. La búsqueda con tres variantes recuperó 4 fragmentos relevantes adicionales que la query original no encontraba, al usar vocabulario diferente al presente en el documento.
    \item \textbf{Prueba C --- Consulta factual:} ``¿Cuándo caduca el certificado ISO 9001?''
    \item \textbf{Resultado:} Clasificada como \texttt{FACTUAL}, usó \texttt{top\_k=5} y modelo JARVIS. Latencia total $<3$\,s incluyendo la expansión (timeout 12\,s previene bloqueo si LiteLLM está ocupado).
\end{itemize}

\subsection{Validación final con documentación del TFG}

Para disponer de una prueba reproducible sin capturar documentación corporativa, se creó una carpeta de demostración en el volumen de ingesta: \texttt{data/watch/TFG\_DEMO}. En ella se añadieron cuatro documentos CSV controlados: \texttt{TFG\_DEMO\_01\_arquitectura.csv}, \texttt{TFG\_DEMO\_02\_operacion.csv}, \texttt{TFG\_DEMO\_03\_memoria\_y\_pages.csv} y \texttt{TFG\_DEMO\_04\_accesos\_demo.csv}. Posteriormente se indexaron en la colección aislada \texttt{documents\_TFGDEMO} mediante el endpoint \texttt{POST /documents/upload} con la cabecera \texttt{X-Tenant-Id: documents\_TFGDEMO}.

Esta carpeta es también la referencia práctica para añadir documentos de prueba al RAG durante una validación local: los ficheros se colocan bajo \texttt{data/watch/} y se procesan por el Indexer o por la API de carga del Backend, indicando la colección de destino cuando se necesita separar el contenido por permisos o por entorno. En esta ejecución, la colección generó 13 puntos vectoriales y 4 documentos lógicos.

Para facilitar la reproducción de la demo, los accesos utilizados fueron cuentas locales y contraseñas dummy, sin validez fuera del entorno de pruebas: OpenWebUI con \texttt{admin@tfg.local} / \texttt{Admin123456!}, Grafana con \texttt{admin} / \texttt{dummy\_grafana\_password}, MinIO con \texttt{minio\_admin} / \texttt{dummy\_minio\_password}, pgAdmin con \texttt{admin@example.com} / \texttt{admin} y LiteLLM mediante la clave local \texttt{dummy\_litellm\_key}. En un despliegue real estas credenciales deben sustituirse por secretos gestionados y no deben publicarse.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{cap_tfg_documentos_demo_backend.png}
\caption[Listado real de documentos cargados en la colección de demo.]{Listado real de documentos cargados en \texttt{documents\_TFGDEMO}. La captura procede del endpoint \texttt{GET /documents/list} y muestra los documentos, sus chunks, la extracción nativa y el instante de ingesta.}
\label{fig:tfgdemo-documentos}
\end{figure}

Sobre esa colección se lanzó una consulta RAG de control preguntando por los puertos del entorno limpio del TFG y el papel de Qdrant. La respuesta recuperó fuentes de la colección \texttt{documents\_TFGDEMO}, mantuvo trazabilidad hacia los documentos de origen y devolvió una respuesta sin apoyarse en conocimiento externo.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{cap_tfg_backend_chat_respuesta.png}
\caption[Respuesta RAG real sobre la colección de demostración.]{Respuesta RAG real sobre la colección de demostración. La captura muestra la pregunta, la respuesta generada por el Backend y las fuentes recuperadas con sus puntuaciones de relevancia.}
\label{fig:tfgdemo-rag}
\end{figure}

La misma consulta se ejecutó desde OpenWebUI mediante el modelo \texttt{JARVIS}. Esta prueba valida el recorrido completo usuario--interfaz--pipeline--backend--Qdrant--LLM y demuestra que la respuesta visible en la aplicación mantiene la misma trazabilidad documental.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{cap_tfg_openwebui_chat_rag.png}
\caption[Consulta RAG desde OpenWebUI durante la demo final.]{Consulta RAG desde OpenWebUI durante la demo final. JARVIS responde con los puertos del entorno limpio, explica el papel de Qdrant y muestra las fuentes documentales utilizadas.}
\label{fig:tfg-openwebui-rag}
\end{figure}

Como apoyo a la defensa, se actualizó también la página de GitHub Pages del proyecto con el nombre final \textit{JARVIS RAG}, una sección de capturas reales y un vídeo \texttt{webm} de demostración que recorre Pages, Swagger, Qdrant, OpenWebUI y Grafana.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{cap_tfg_github_pages_demo_video.png}
\caption[Sección de demo publicada en GitHub Pages.]{Sección de demo publicada en GitHub Pages. La página incluye el vídeo de demostración y las métricas principales del sistema.}
\label{fig:tfg-pages-demo}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Solapamiento de ventanas contextuales:} El solapamiento (50 tokens sobre 500, $\sim$10\%) en el chunking asegura que el hilo semántico de párrafos extensos no se pierde en los límites de fragmento, proporcionando al modelo contexto suficiente para generar respuestas coherentes.
\end{enumerate}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{screenshot_rag_response_no_documents.png}
\caption[Comportamiento del sistema sin documentos disponibles.]{Comportamiento del sistema cuando no existen documentos disponibles para el usuario. En lugar de inventar una respuesta, JARVIS informa explícitamente de la ausencia de documentos consultables y muestra las colecciones evaluadas.}
\label{fig:no-documentos-disponibles}
\end{figure}
