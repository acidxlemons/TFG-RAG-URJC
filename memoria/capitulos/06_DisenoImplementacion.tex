...
Este capítulo constituye el núcleo técnico de mi Trabajo de Fin de Grado. La idea de fondo que conviene tener presente al leerlo es sencilla: el sistema no es una única aplicación escrita de principio a fin, sino un \textbf{conjunto de piezas integradas}. Algunas son herramientas de código abierto ya existentes que he reutilizado ---empezando por las dos sobre las que se apoya todo lo demás: \textbf{Ollama}, que ejecuta los modelos de lenguaje en local, y \textbf{OpenWebUI}, que aporta la interfaz de chat---; otras son desarrollo propio ---el \textit{pipeline} de encaminamiento, el backend RAG, el servicio de indexación, el agente SQL y las integraciones con cada fuente---. El trabajo de este TFG consiste precisamente en diseñar esa arquitectura y en escribir el código que conecta unas piezas con otras hasta convertir componentes sueltos en un asistente corporativo coherente.

Por eso el capítulo se lee mejor como un recorrido: parte del sistema mínimo ---un simple chat local montado sobre Ollama y OpenWebUI--- y va explicando, paso a paso, qué pieza se añadió después y por qué, distinguiendo en cada caso qué es reutilización de una herramienta existente y qué es aportación propia. Para no recargar la lectura, el detalle de implementación más extenso ---fragmentos de código, contratos de clase y flujos paso a paso--- se traslada a los Anexos~\ref{anexo:pipeline}--\ref{anexo:finetuning}, y aquí se mantiene la visión de arquitectura y las decisiones de diseño.
...
\begin{enumerate}
    ...
    \item Novena limitación: El control de acceso mediante cabeceras HTTP era insuficiente para entornos multi-tenant (múltiples grupos o departamentos con documentación aislada) exigentes. \textit{Solución:} Se implementó validación criptográfica de tokens JWT (\textit{JSON Web Token}) de Azure AD (\texttt{core/auth.py}), con descarga dinámica de las claves públicas del proveedor (JWKS, \textit{JSON Web Key Set}), activable con \texttt{AZURE\_JWT\_VALIDATION=true}.
...
\caption[Estado del stack Docker en la validación final.]{Estado del stack Docker en la validación final. La captura muestra los servicios principales de \texttt{TFG-RAG-Clean}, su estado de ejecución y los puertos alternativos publicados para la demo local.}
...
\caption[Documentación interactiva del Backend FastAPI.]{Documentación interactiva del Backend FastAPI en el entorno local de validación: API activa, endpoints de scraping, búsqueda y salud, y contrato OpenAPI expuesto.}
...
El aspecto real de esta interfaz en funcionamiento, con la respuesta y sus fuentes, se mostró en la Figura~\ref{fig:screenshot-openwebui} del Capítulo~\ref{cap:funcional}.
...
\caption[Respuesta real del modo de listado documental.]{Respuesta real del modo de listado documental. JARVIS consulta el estado del indexador y devuelve los documentos disponibles, junto con la información reciente de ingesta.}
\label{fig:documentos-disponibles}
\end{figure}

\paragraph{Reglas frente a un router basado en LLM.} La decisión de qué hacer con cada mensaje se toma, por tanto, mediante \textbf{reglas deterministas}, sin delegarla a un modelo de lenguaje. Es una elección de diseño deliberada: el enrutamiento por reglas es prácticamente instantáneo ($\sim$10\,ms), no consume tokens ni GPU, es reproducible y se puede validar de forma sistemática (alcanzó un 92\% de acierto, véase el Capítulo~\ref{cap:resultados}). A ello se añade que el modelo principal (\texttt{rag-qwen-ft}) se expone sin \textit{function calling}, por lo que no podría actuar como router fiable. La alternativa ---delegar la decisión en un LLM que actúe como \textit{agente router}, enfoque adoptado en la variante empresarial derivada de este TFG--- es más robusta ante fraseos inesperados, pero añade latencia, coste por consulta y pérdida de determinismo. Por qué no se ha integrado aquí y cómo podría hacerse (incluido un esquema híbrido) se detalla en el Anexo~\ref{anexo:pipeline}.
...
La \textbf{ingesta} de un documento sigue seis etapas: recepción (API, Indexer o scraper), detección de tipo (texto nativo vs. escaneado), extracción de texto (\texttt{pdfplumber}, PaddleOCR o la cadena Trafilatura/Readability/BeautifulSoup), \textit{chunking} semántico (fragmentos de unos 500 tokens con 50 de solapamiento, $\sim$10\%), generación de \textit{embeddings} (vector denso de 384 dimensiones con \path{paraphrase-multilingual-MiniLM-L12-v2}) y almacenamiento en Qdrant con metadatos. El procedimiento completo se detalla en el Anexo~\ref{anexo:backend}.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{cap_tfg_qdrant_dashboard.png}
\caption[Estado de las colecciones en Qdrant durante la demo final.]{Estado de las colecciones en Qdrant durante la demo final. La colección \texttt{documents\_TFGDEMO} ---documentación sintética y segura del propio TFG--- aparece separada del resto, con 13 puntos y configuración híbrida dense+sparse.}
...
\caption[Pantalla de autenticación de OpenWebUI.]{Pantalla real de autenticación de OpenWebUI en el despliegue local: inicio de sesión clásico y acceso mediante SSO corporativo.}
\label{fig:login-sso}
\end{figure}
...
\caption[Dashboard principal de Grafana durante la validación final.]{Dashboard principal de Grafana durante la validación final: estado operativo del backend, Qdrant y Prometheus, además de métricas de memoria, CPU y ficheros abiertos.}
